...
Были сложности при запуске хорошего compose-файла после плохого. Проблема была в том, что база данных в хорошем не поднималась вместе с приложением. А дело же было в том, что при сборке плохого compose-файла использовалась последняя версия базы данных 17, а при сборке хорошего -- уже конкретная версия 13.4, и при этом они оба использовали одну и ту же директорию для хранения базы данных, и в ней уже после запуска плохого была версия 17, из-за чего возникал конфликт с версией 13.4 у хорошего. Это было решено с помощью разделения директорий для хорошего и для плохого директивой `volumes`.
